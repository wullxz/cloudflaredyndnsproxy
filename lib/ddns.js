...
module.exports = function() {
  ...
  proto.deleteDNSRecord = function (name) {
    var self = this;
    return new Promise(function (resolve, reject) {
      self.getDNSRecord(name).then(function (data) {
        if (data.isNew) {
          console.log("Domain " + data.record.name + " does not exist. Not deleting!");
          resolve({ success: true });
        }
        else {
          console.log("Deleting domain " + domain.record.name);
          self.api.deleteDNS(data.record)
            .then(resolve)
            .catch(reject);
        }
      }).catch(reject);
